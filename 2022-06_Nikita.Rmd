...
astra$scale_age <- scale(astra$age, scale = F)

astra$scale_vax <- scale(astra$vax, scale = F)

mod1_astra <- lm(willingness~scale_vax + scale_age*frame, data=astra)

## Check residuals etc

visualize(mod1_astra, plot="residuals")

## Check more robust model

mod1.2_astra = rlm(willingness~scale_vax + scale_age*frame, data=astra)

## Compare visualisations of each robust and normal

visualize(mod1_astra, plot="model")

visualize(mod1.2_astra , plot="model") ### Look the same... 

summary(mod1_astra)

smallmod_astra = lm(willingness~1, data=astra)

model.comparison(smallmod_astra, mod1_astra)

compare.fits(willingness~age | frame, 
             data=astra, 
             model1 = mod1_astra,
             model2 = smallmod_astra)

estimates(mod1_astra)

### Alternative hypothesis 1

astra$scale_bis <- scale(astra$bis, scale = F)

astra$scale_bas <- scale(astra$bas, scale = F)

mod2_astra = lm(willingness~scale_vax + scale_bis*scale_age*frame + scale_bas*scale_age*frame, data = astra)

visualize(mod2_astra, plot="residuals")

model.comparison(smallmod_astra, mod2_astra)
